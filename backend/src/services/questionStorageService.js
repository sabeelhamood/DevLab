...
export const saveQuestionToSupabase = async (questionData, metadata = {}) => {
  try {
    ...
    if (!resolvedCourseId && (metaCourseName || metadata.course_name)) {
      ...
      try {
        ...
        if (courseLookup.rows.length > 0) {
          ...
          console.log(`✅ Found course: ${resolvedCourseId} for course_name: "${courseNameToSearch}"`)
        }
      } catch (courseError) {
        console.warn(`⚠️ Could not lookup course by name: ${courseError.message}`)
      }
    }
    ...
    if (!resolvedCourseId && DEFAULT_COURSE_ID) {
      ...
      console.log(`✅ Using DEFAULT_COURSE_ID: ${resolvedCourseId}`)
    }
    
    // Log warning if course_id is still missing (will prevent topic creation)
    if (!resolvedCourseId) {
      console.warn(`⚠️ Cannot create topic: course_id missing — skipping topic creation`)
        ...
        try {
          ...
          if (topicLookup.rows.length > 0) {
            ...
            if (!resolvedCourseId && topicLookup.rows[0].course_id) {
              ...
              console.log(`✅ Extracted course_id from topic: ${resolvedCourseId}`)
            }
            console.log(`✅ Found topic UUID: ${topicIdUuid} for topic_name: "${topicNameToSearch}"`)
    ...
    try {
      ...
      if (duplicateCheck.rows.length > 0) {
        console.log(`⚠️ Duplicate question found, skipping insertion: ${duplicateCheck.rows[0].question_id}`)
    ...
    if (insertResult.rows.length === 0) {
      ...
      console.warn(`⚠️ Question ID conflict, using existing question: ${questionId}`)
    ...
    console.log(`✅ Question saved to Supabase: ${savedQuestion.question_id}`)
...
const saveTestCasesToSupabase = async (questionId, testCases) => {
  try {
    ...
    console.log(`🔍 Saving ${testCases.length} test case(s) for question ${questionId}`)
    ...
    for (const testCase of testCases) {
      ...
      if (!testCaseOutput) {
        console.warn('⚠️ Skipping test case without expected output')
        continue
      }
      ...
      if (testCaseResult.rows.length > 0) {
        console.log(`   ✅ Test case saved: ${testCaseResult.rows[0].testCase_id}`)
      }
    }
  } catch (error) {
...
export const saveQuestionsToSupabase = async (questions, metadata = {}) => {
  ...
  console.log(`🔍 Saving ${questions.length} question(s) to Supabase...`)
  ...
  for (let i = 0; i < questions.length; i++) {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log(`   ⏭️  Question ${i + 1} skipped (duplicate)`)
      } else {
        console.log(`   ⚠️  Question ${i + 1} not saved: ${result.message || 'Unknown error'}`)
      }
    } catch (error) {
