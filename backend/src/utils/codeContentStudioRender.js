      ...
      (function () {
        ...
        const restoreCodeForCurrentQuestion = () => {
          ...
          const renderEvaluationCard = (evaluation) => {
            ...
            const specificErrors = Array.isArray(evaluation.specificErrors)
              ? evaluation.specificErrors
            ...
            let specificErrorsHtml = '';
            if (specificErrors.length) {
              const items = specificErrors.slice(0, 3).map(err => {
                const text = typeof err === 'string' ? err : String(err);
                return (
                  '<li style="font-size:13px;line-height:1.5;color:#7f1d1d;">' +
                  text +
                  '</li>'
                );
              }).join('');
                ...
                onConfirm: async ({ overlay, status }) => {
                  try {
                    ...
                    const solutionCard =
                      ...
                      '</div>' +
                      '</div>' +
                      '<div style="padding:20px 22px 22px 22px;background:#f8fafc;">' +
                      '<pre style="margin:0;border-radius:14px;background:#f1f5f9;color:#0f172a;font-family:\'JetBrains Mono\',\'Fira Code\',monospace;font-size:0.95rem;line-height:1.75;white-space:pre-wrap;word-break:break-word;padding:18px 20px;border:1px solid rgba(15,23,42,0.08);box-shadow: inset 0 1px 2px rgba(15,23,42,0.04);">' +
